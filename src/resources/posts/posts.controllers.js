...
export async function createPostByUser(req, res) {
  try {
    ...
    const result = await query('INSERT INTO posts (title, content, userId) VALUES (?, ?, ?)', [
      title,
      content,
      userId,
    ]);

    if (result.affectedRows < 1) return res.status(400).json({ error: 'Post not created!' });
...
export async function updatePost(req, res) {
  try {
    ...
    const result = await query('UPDATE posts SET title = ?, content = ? WHERE id = ?', [
      title,
      content,
      id,
    ]);

    if (result.affectedRows < 1) return res.status(404).json({ error: 'Post not updated!' });
...
export async function deletePost(req, res) {
  try {
    ...
    if (result.affectedRows < 1) return res.status(404).json({ error: 'Post not deleted!' });
